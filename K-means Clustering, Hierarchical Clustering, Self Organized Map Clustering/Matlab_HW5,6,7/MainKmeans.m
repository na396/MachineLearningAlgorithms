...
for k=firstt : second
    first = counter;
    for u=1 : 5
        [iteration,clusters(: , :),numberinclster] =kmeans(data,k);
        k
        u
        numberinclster
        iteration
    ...
    range = first : first+4;
    figure
    title('evaluations for kmeans Davies Bouldin Index: red and S Dbw Validity Index: blue') ;
    xlabel('run')
    ylabel('evaluation')
