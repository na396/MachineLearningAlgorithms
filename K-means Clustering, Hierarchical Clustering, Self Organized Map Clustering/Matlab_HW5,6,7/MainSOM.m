...
counter = 1;
%evaluttionmatrix = zeros(30,2);
iterationSOM = zeros(30,2);
run= 1:5;
range = 0;
first=0;
for k=firstt : second
    first = counter;
    for u=1 : 5
        [iteration,clusters(: , :),numberinclster] =SOM(data,k);
        k
        u
        numberinclster
        iteration
        iterationSOM(counter)=iteration;  
    ...
    range = first : first+4;
    figure
    title('evaluations for SOM Davies Bouldin Index: red and S Dbw Validity Index: blue') ;
    xlabel('run')
    ylabel('evaluation')
